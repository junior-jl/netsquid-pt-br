...
class ConexaoClassica(Connection):
    def __init__(self, comprimento):
        ...
        self.ports["A"].forward_input(self.subcomponents["Canal_AparaB"].ports["send"])
        self.subcomponents["Canal_AparaB"].ports["recv"].forward_output(self.ports["B"])
...
conexaoc.add_subcomponent(
    canalc, forward_input=[("A", "send")], forward_output=[("B", "recv")]
)
...
def configura_rede_exemplo(node_distance=4e-3, depolar_rate=1e7):
    ...
    alice = Node(
        ...
        qmemory=QuantumMemory(
            "MemoriaAlice", num_positions=2, memory_noise_models=[modelo_ruido] * 2
        ),
    )
