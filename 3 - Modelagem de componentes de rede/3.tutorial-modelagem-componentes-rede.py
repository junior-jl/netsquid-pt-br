...
canalq = QuantumChannel(
    "MeuCanalQ", length=20, models={"quantum_loss_model": modelo_perda}
)
...
print(
    f"Medição na posição 0 da memória (memq.measure(positions=0)):\n"
    f" {memq.measure(positions=0)}"
)
# Também é possível mudar a base de medição:
print(
    f"Medição na posição 0 na base X (memq.measure(positions=0, observable=ops.X)):\n"
...
ns.sim_run()
print(
    f"Posição de memória 0 da memória conectada ao canal pela porta qin0: {memq.peek(0)}"
)
...
class EntidadePing(pydynaa.Entity):
    ...
    def __init__(self):
        ...
        # evtype_input
        self._wait(
            pydynaa.EventHandler(self._lida_com_qubit_entrada),
            entity=self.memoriaq.ports["qin0"],
    ...
    def _lida_com_qubit_entrada(self, evento):
        ...
        labels_z = ("|0>", "|1>")
        print(
            f"{ns.sim_time():.1f}: Evento Pong! A EntidadePing mediu "
...
class EntidadePong(pydynaa.Entity):
    ...
    def _lida_com_qubit_entrada(self, evento):
        ...
        labels_x = ("|+>", "|->")
        print(
            f"{ns.sim_time():.1f}: Evento ping! EntidadePong mediu "
...
class Alice(pydynaa.Entity):
    def __init__(self, estado_teleporte, canalc_porta_send):
        ...
        self._wait(
            pydynaa.EventHandler(self._lida_com_qubit_entrada),
            entity=self.memoriaq.ports["qin1"],
            event_type=Port.evtype_input,
        )
    ...
    def _lida_com_qubit_entrada(self, evento):
        ...
        m0, m1 = self.memoriaq.measure(positions=[0, 1], observable=ns.Z, discard=True)[
            0
        ]
        ...
        separador()
        print(
            f"{ns.sim_time():.1f}: Alice recebeu o qubit emaranhado, "
...
class Bob(pydynaa.Entity):
    ...
    def __init__(self, canalc_porta_recv):
        ...
        self.memoriaq = QuantumMemory(
            "MemoriaBob", num_positions=1, memory_noise_models=[modelo_ruido]
        )
    ...
    def _lida_com_correcoes(self, mensagem):
        ...
        separador()
        print(
            f"{ns.sim_time():.1f}: Bob recebeu o qubit emaranhado e as correções!"
            f" Fidelidade = {fidelidade:.3f}"
        )
        print(f"Qubit de Bob após aplicação das correções: " f"{qubit[0].qstate.qrepr}")
...
def configura_rede(alice, bob, fonteq, comprimento=4e-3):
    canalq_c_para_a = QuantumChannel(
        "Charlie->Alice",
        length=comprimento / 2,
        models={"delay_model": FibreDelayModel()},
    )
    canalq_c_para_b = QuantumChannel(
        "Charlie->Bob",
        length=comprimento / 2,
        models={"delay_model": FibreDelayModel()},
    )
...
canalc = ClassicalChannel(
    "CanalC", length=4e-3, models={"delay_model": FibreDelayModel()}
)
