...
class ProtocoloPing(NodeProtocol):
    def run(self):
        ...
        while True:
            ...
            labels_z = ("|0>", "|1>")
            print(
                f"{ns.sim_time()}: Evento pong! {self.node.name} mediu "
...
class ProtocoloPong(NodeProtocol):
    def run(self):
        ...
        while True:
            ...
            labels_x = ("|+>", "|->")
            print(
                f"{ns.sim_time()}: Evento ping! {self.node.name} mediu "
    ...
    def run(self):
        ...
        while True:
            exprev_sinal = self.await_signal(
                sender=self.subprotocols["protocoloq"], signal_label=Signals.SUCCESS
            )
        ...
        while True:
            ...
            if resultados_medicao is not None and emaranhamento_pronto:
                ...
                fidelidade = ns.qubits.fidelity(
                    self.node.qmemory.peek(0)[0], ns.y0, squared=True
                )
                print(
                    f"{ns.sim_time():.1f}: Bob recebeu o qubit emaranhado e "
