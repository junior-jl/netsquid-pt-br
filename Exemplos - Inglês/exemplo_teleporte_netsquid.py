...
class CorrectionProtocol(NodeProtocol):
    ...
    def run(self):
        ...
        while True:
            ...
            expr = yield (
                self.await_port_input(port_alice) | self.await_port_input(port_charlie)
            )
...
def example_sim_setup(node_A, node_B):
    ...
    dc.collect_on(
        pydynaa.EventExpression(source=protocol_bob, event_type=Signals.SUCCESS.value)
    )
...
def create_plot():
    ...
    data = (
        fidelities.groupby("depolar_rate")["fidelity"]
