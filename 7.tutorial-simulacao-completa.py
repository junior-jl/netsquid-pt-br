...
def cria_processador(taxa_depolar, taxa_defasagem):
    modelo_ruido_medicao = DephaseNoiseModel(dephase_rate=taxa_defasagem,
    ...
    instrucoes_fisicas = [
        ...
        PhysicalInstruction(instr.INSTR_MEASURE, duration=7, parallel=False, topology=[0],
                            quantum_noise_model=modelo_ruido_medicao, apply_q_noise_after=False),
    ...
    processador = QuantumProcessor("processador_quantico", num_positions=2,
...
def configura_rede_exemplo(distancia_nos=4e-3, taxa_depolar=1e7, taxa_defasagem=0.2):
    ...
    rede.add_connection(alice, bob, connection=con_c, label="classica",
    ...
    porta_ac, porta_bc = rede.add_connection(
            alice, bob, connection=con_q, label="quantica",
...
# programa finalize ou que tenha recebido o qubit de Charlie na sua memória.
...
class ProtocoloCorrecao(NodeProtocol):
    def run(self):
        ...
        while True:
            ...
            expr = yield (self.await_port_input(porta_alice) | 
...
def roda_experimento(numero_rodadas, taxas_depolar, distancia=4e-3, taxa_defasagem=0.0):
    ...
    for i, taxa_depolar in enumerate(taxas_depolar):
        ...
        cycle_runtime = (con_q.subcomponents["fonteq"].subcomponents["internal_clock"]
...
def cria_plot():
    ...
    fidelidades = roda_experimento(numero_rodadas=1000, distancia=4e-3,
            taxas_depolar = taxas_depolar, taxa_defasagem=0.0)
    estilo = {'kind': 'scatter', 'grid': True,
            'title': 'Fidelidade do estado quântico teleportado'}
    dados = fidelidades.groupby('taxa_depolar')['fidelidade'].agg(
            fidelidade='mean', sem='sem').reset_index()
