...
class EntidadePing(pydynaa.Entity):
    ...
    def espera_por_pong(self, entidade_pong):
        ...
        self._wait(
            manipulador_pong, entity=entidade_pong, event_type=EntidadePong.tipo_ev_pong
        )
    ...
    def _lida_com_evento_pong(self, evento):
        ...
        print(
            f"{ns.sim_time():.1f}: Evento pong! EntidadePing mediu "
...
class EntidadePong(pydynaa.Entity):
    ...
    def espera_por_ping(self, entidade_ping):
        ...
        self._wait(
            manipulador_ping, entity=entidade_ping, event_type=EntidadePing.tipo_ev_ping
        )
...
class Charlie(pydynaa.Entity):
    tipo_ev_pronto = pydynaa.EventType(
        "QUBITS_PRONTOS", "Qubits emaranhados estão prontos."
    )
    ...
    def __init__(self):
        ...
        self._wait(
            self._manipulador_geracao, entity=self, event_type=Charlie._tipo_ev_gerar
        )
...
class Alice(pydynaa.Entity):
    tipo_ev_pronto = pydynaa.EventType(
        "CORRECOES_PRONTAS", "As correções estão prontas."
    )
    ...
    def espera_por_charlie(self, charlie):
        ...
        self._wait(
            self._manipulador_qubit, entity=charlie, event_type=Charlie.tipo_ev_pronto
        )
...
class Bob(pydynaa.Entity):
    def espera_por_teleporte(self, alice, charlie):
        ...
        expressao_ev_ambos_prontos = (
            expressao_ev_charlie_pronto & expressao_ev_alice_pronto
        )
        self._manipulador_teleporte = pydynaa.ExpressionHandler(
            self._lida_com_teleporte
        )
    ...
    def _aplica_correcoes(self, qubit, correcoes):
        ...
        print(
            f"{ns.sim_time():.1f}: Bob recebeu o qubit emaranhado e correções!"
